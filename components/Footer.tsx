    ...
    return (
      <footer className="bg-gradient-dark py-6">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-white/60">
          <span>&copy; {new Date().getFullYear()} {siteConfig.business.name}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div>
            ...
            <p className="text-sm text-white/60">{siteConfig.hours.displayText}</p>
          </div>

          <div>
            <p className="text-xs font-semibold text-white uppercase tracking-widest mb-4">Quartiers desservis</p>
            <ul className="space-y-2">
              {neighborhoods.slice(0, 6).map((neighborhood) => (
                <li key={neighborhood.slug}>
                  <Link href={`/electricien-${neighborhood.slug}`} className="text-sm text-white/60 hover:text-white transition-colors">
                    {neighborhood.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <p className="text-xs text-white/40 mt-4">{siteConfig.business.licenseLabel}: {siteConfig.business.licenseNumber}</p>
          </div>
        </div>

        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-white/60">
            &copy; {new Date().getFullYear()} {siteConfig.business.name}. Tous droits réservés.
          </p>
          <a
            href={MAIN_PHONE_TEL}
